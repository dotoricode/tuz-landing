...
import {
  ...
  listArts, createArt, toggleArt, deleteArt, getMyCoupons,
} from './stamps.js';

const COUPON_LABEL = {
  half_off:   { label: '반값 쿠폰',    sub: '다음 음료 50% 할인' },
  free_drink: { label: '무료 음료권',  sub: '카운터에 알려주세요' },
};
...
function loginButtonHtml({ style = '' } = {}) {
  return `
    <div style="display:flex;flex-direction:column;gap:8px;width:100%">
      <button class="btn btn--primary" type="button" data-tuz-action="kakao-login">카카오로 로그인</button>
      <div class="email-login-wrap">
        <form data-tuz-form="email-login" style="display:flex;gap:6px">
          <input type="text" name="email" placeholder="이메일로 로그인 (테스트용)"
            style="flex:1;min-width:0;padding:8px 10px;border:1px solid var(--line);border-radius:8px;background:var(--paper);color:var(--ink);font-size:13px">
          <button class="btn btn--ghost" type="submit" style="flex:0 0 auto;white-space:nowrap;font-size:13px">OTP 전송</button>
        </form>
        <form data-tuz-form="password-login" style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:6px">
          <input type="text" name="email" placeholder="이메일"
            style="padding:8px 10px;border:1px solid var(--line);border-radius:8px;background:var(--paper);color:var(--ink);font-size:13px">
          <input type="password" name="password" placeholder="비밀번호"
            style="padding:8px 10px;border:1px solid var(--line);border-radius:8px;background:var(--paper);color:var(--ink);font-size:13px">
          <button class="btn btn--ghost" type="submit" style="grid-column:1/-1;font-size:13px">로그인</button>
        </form>
...
const TIER_INFO = {
  normal:     { label: '도장 +1',       sub: '잘 오셨어요 :)', color: '#d4a574', start: 0,     end: 306,   center: 153   },
  double:     { label: '더블 도장!',    sub: '오늘 운 좋은 날이에요', color: '#a52a1a', start: 306,  end: 342,   center: 324   },
  half_off:   { label: '반값 쿠폰',     sub: '다음 음료 50% 할인', color: '#5c7a4e', start: 342,  end: 356.4, center: 349.2 },
  free_drink: { label: '무료 음료권!',  sub: '카운터에 알려주세요', color: '#2d5016', start: 356.4, end: 360, center: 358.2 },
};
...
async function renderStampView() {
  ...
  const count = await refreshActiveCount();
  const [{ stamps, rewards }, arts, coupons] = await Promise.all([
    getMyHistory(40),
    listArts(),
    getMyCoupons().catch(() => []),
  ]);
  ...
  root.innerHTML = `
    ...
    ${coupons.length ? `
    <div class="card">
      <div class="eyebrow" style="margin-bottom:10px">내 쿠폰 (${coupons.length})</div>
      <div class="coupon-list">
        ${coupons.map((c) => {
          const info = COUPON_LABEL[c.coupon_type] || { label: c.coupon_type, sub: '' };
          return `
            <div class="coupon-item coupon-item--${esc(c.coupon_type)}">
              <div class="coupon-item__body">
                <strong class="coupon-item__label">${esc(info.label)}</strong>
                <span class="coupon-item__sub">${esc(info.sub)}</span>
              </div>
              <span class="coupon-item__date">${esc(formatDateShort(c.issued_at))}</span>
            </div>
          `;
        }).join('')}
      </div>
      <p style="font-size:11px;color:var(--ink-3);margin:8px 0 0">카운터에 이 화면을 보여주시면 적용해 드립니다</p>
    </div>
    ` : ''}
    ...
    <div class="card">
      ...
      <div class="stamp-history">
        ${[
          ...rewards.map((r) => ({ kind: 'reward', date: r.redeemed_at, label: '보상 교환', bonus: null })),
          ...stamps.map((s) => ({
            kind: s.redeemed_at ? 'used' : 'active',
            date: s.created_at,
            label: s.note || '스탬프 적립',
            bonus: s.bonus_type && s.bonus_type !== 'normal' ? s.bonus_type : null,
          })),
        ].sort((a, b) => String(b.date).localeCompare(String(a.date))).slice(0, 30).map((row) => {
          const tierInfo = row.bonus ? TIER_INFO[row.bonus] : null;
          const bonusPill = tierInfo
            ? `<span class="stamp-history__bonus" style="background:${tierInfo.color}">${esc(tierInfo.label)}</span>`
            : '';
          return `
          <div class="stamp-history__row">
            <span class="stamp-history__date">${esc(formatDateShort(row.date))}</span>
            <span>${esc(row.label)}${bonusPill}</span>
